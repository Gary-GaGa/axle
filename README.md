...
| 🔧 自我升級 | 描述需求 → AI 規劃 → 自動修改程式碼 → 編譯測試 → 重啟更新 |
...
Axle 具備自我進化能力——直接透過 Telegram 對話即可新增功能：

1. 啟用「🔧 自我升級」（在「➕ 更多功能」中開啟）
2. 描述想要的功能（例如「加一個天氣查詢功能」）
3. Axle 透過 Copilot CLI 分析原始碼，產出升級計畫
4. 確認後自動：修改程式碼 → 遞增版本號 → 編譯 → 測試 → Git commit → 重啟

**安全機制：**
- 每次升級前需人工確認計畫
- 編譯或測試失敗自動回滾至舊版 binary
- 備份檔保留於 `axle.bak`
- 版本號自動遞增（SemVer patch）
